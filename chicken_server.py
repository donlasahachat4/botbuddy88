...
app = Flask(__name__)
app.config['JWT_SE0CRET_KEY'] = 'aa123456'
jwt = JWTManager(app)
CORS(app, resources={r"/*": {"origins": "*"}})
socketio = SocketIO(app, cors_allowed_origins='*')

#domain = "https://cus3.witheeit.xyz"
domain = "https://cus3.witheeit.xyz/"
...
#Channel_access_token = "PLlfRCJH22wws+4iR0Nn7WeqIGTNiCWdXVV4F29O2PRhY2Bs/VutyURnaN3F5HC0/DqUyQ0ZMFFghZrZ+BNV34F4IrKbKa/mC8HovlB/3rbBLH1vjWRLqhILIWiSaD148fBtOm2RP2ZMA9x2rmHuKQdB04t89/1O/w1cDnyilFU=" #cus 3
Channel_access_token ="ssFF9TVB9hE2VwcHPKBPhWKHPJ/MnFSy33vVfi7ThZ8ZBVdU2+ZvYR9ubChpN2Z+ARTLNOjKtwV3pCVBx8LKIrvcn/dZiNqPZlaF5cEsHsbx220Ze16Z3ShhPGXrlSWy6nArhS9vQIwFAAxh/NFeSgdB04t89/1O/w1cDnyilFU=" #cus4


#line_notify_token = 'WFJAUkO7gFZMQUt6k3qpMKbdhNubVHbD7K7hoo81rmd'
#line_notify_token = 'BxIZDd6bnTXnNeJm1BjeH9IOKTYqAhcJgtPYooUZDlL' #cus3
line_notify_token = "cLxbKMy3t6qwFA1Kj8Ar9nGdCM3QxnheG4Z2GmmGcYB"#cus4
# possibilities = ['p1','p2','p3','p4']

#http_proxy = "http://PVN5CfQ5D1jsfoPE:z6kX9By593dp5Qex_country-th@geo.iproyal.com:12321"
#http_proxy = "http://PVN5CfQ5D1jsfoPE:z6kX9By593dp5Qex_country-th_session-5fZ0FdgM_lifetime-24h_streaming-1@geo.iproyal.com:12321"
http_proxy=''

db_name = 'chicken_cus4.db'

link_acc_room = """ห้องฝาก-ถอน\nxxx\n👈จิ้มเลย ลิ้งเข้าห้องฝาก"""

link_play_room = """ห้องเล่น \n xx\n👈จิ้มเลย ลิ้งเข้าห้องเล่น"""

acc_number = '4211906483'
...
main_room = 'Ccb96d34aab8be99721389706dc706752'
...
LINK_ROOM = "https://line.me/R/ti/g/BCmm5VAxuq"
...
def get_img_setting(d=0):
    global link_acc_room,link_play_room,acc_pic,game_name,ac_msg,open_pic,close_pic,red_win,blue_win,tie,how_to_play_pic,msg_how_to_play,acc_num,acc_number
    if d == 0:
        with sqlite3.connect(db_name) as conn:
            cursor = conn.cursor()
            ...
            msg_how_to_play=data[11].replace('\\','/')
            acc_num = data[12].replace('\\','/')
            acc_number = data[12].replace('\\','/')
            return setting
    elif d == 1:
        link_acc_room="""ห้องฝาก-ถอน\nxxx\n👈จิ้มเลย ลิ้งเข้าห้องฝาก"""
        link_play_room="""ห้องเล่น \n xx\n👈จิ้มเลย ลิ้งเข้าห้องเล่น"""
        acc_pic=f"pic/ac5.jpg"
        game_name='ไก่ชน88'
        ac_msg="""ฝาก ห้ามนำสลิปส่งในกลุ่ม\nให้แจ้งผ่านไลน์นี้เท่านั้น\n👇🏻👇🏻👇🏻👇🏻👇🏻\nhttps://lin.ee/7V4vQtu"""
        open_pic=f"pic/open.jpg"
        close_pic=f"pic/close.jpg"
        red_win=f"pic/red.jpg"
        blue_win=f"pic/blue.jpg"
        tie=f"pic/tie.jpg"
        how_to_play_pic=f"pic/how_to_play.jpg"
    ...
    try:
        if request.method == 'POST':
            ...
            for event in payload['events']:
                ...
                if user_id == "U924c90198e3dd75025f5db62029ab398" or user_id == "Uafa0e3dc0a95041ee055024ef73d989d" or user_id == 'Ua58aec9a4c24fc4bf1693819028e6dc0':
                    if role[0]=='user':
                ...
                if role[0] == 'admin':
                    ...
                    if "!add_group:" in msg:
                        ...
                        main_room = group_id
                        reply_message = "ตั้งค่าห้องนี้เป็นห้องหลัก"
                        send_reply(reply_token,reply_message)    
                        continue    
                    elif "!ntf:" in msg:
                        room_id =  msg.split(':')[1]
                ...
                if status[1] == 'play_room':
                    ...
                    if role and role[0] == 'admin':
                        ...
                        if '$' in msg:
                            ...
                            pic = f"{domain}/pictures/rr.jpg"
                            message = 'รอราคา'
                            send_msg_pic(reply_token,message,pic)
                            ...
                            try:
                                    ...
                                    if status[2] == 'P':
                                        ...
                                        if len(data) ==4:
                                            try:
                                                ...
                                                send_line_notify(f"\nคำนวณได้เสีย\nรอบที่ {round_value}\nฝั่งแดง #{sp1}{int(user_totals['ด'])}\nฝั่งน้ำเงิน #{sp2}{int(user_totals['ง'])}\nTime:{thailand_time_str}")
                                            except Exception as e:
                                ...
                                if len(data) ==4:
                                    ...
                                    if bet_ratio2 == int(bet_ratio2):
                                        bet_ratio2 = int(bet_ratio2)
                                    fix1 = '10'
                                    fix2 = '10'
                                    sp = 'รอง'
                                    sp1 = 0
                        ...
                        elif 'aa'== msg.lower()[0:2]:
                            game_id = int(msg.lower().replace('aa',''))  
                            try:
                                round_value = get_present_round()
                                with sqlite3.connect(db_name) as conn:
                                    cursor=conn.cursor()
                                    cursor.execute('SELECT user_id,display_name,picture_url FROM user_profiles WHERE id = ?',(game_id,))
                                    profile = cursor.fetchone()
                                    user_id = profile[0]
                                    cursor.execute('SELECT user_id, chicken, bet_ratio, amount_play FROM play WHERE round = ? AND user_id = ?', (round_value,user_id))
                                    records = cursor.fetchall()
                                ...
                                for record in records:
                                    ...
                                    if profile[2]:
                                        ...
                                        profile_pic = """https://www.google.com/imgres?q=google&imgurl=https%3A%2F%2Fyt3.googleusercontent.com%2F2eI1TjX447QZFDe6R32K0V2mjbVMKT5mIfQR-wK5bAsxttS_7qzUDS1ojoSKeSP0NuWd6sl7qQ%3Ds900-c-k-c0x00ffffff-no-rj&imgrefurl=https%3A%2F%2Fwww.youtube.com%2Fchannel%2FUC_x5XG1OV2P6uZZ5FSM9Ttw&docid=hhHmlkfHw9bllM&tbnid=fmDxoTQaoqsYcM&vet=12ahUKEwiuhqiercaNAxWtxjgGHUqcHB8QM3oFCIgBEAA..i&w=900&h=900&hcb=2&ved=2ahUKEwiuhqiercaNAxWtxjgGHUqcHB8QM3oFCIgBEAA"""
                                    bubble = {
                                    "type": "bubble",
                                    'size': 'hecto',
                                    "header": {
                                ...
                                print(e)
                                send_reply(reply_token,e)

                        elif 'ub'== msg.lower()[0:2]:
                            game_id = int(msg.lower().split(':')[0].replace('ub',''))
                            with sqlite3.connect(db_name) as conn:
                                cursor=conn.cursor()
                                cursor.execute('SELECT user_id,display_name,picture_url FROM user_profiles WHERE id = ?',(game_id,))
                                profile = cursor.fetchone()
                                    ...
                                    if notwork != 1: 
                                        ...
                                        if abs(max_minus)> user_balance:
                                            if user_totals['ด']<0  or user_totals['ง'] <0:
                                                ...
                                                continue
                                        if amount < MIN_BET:
                                            reply_message = f'ขั้นต่ำสุดสำหรับการเล่นคือ {MIN_BET} บาท'
                            ...
                            try:
                                with sqlite3.connect(db_name) as conn:
                                    ...
                                    cursor.execute('SELECT double_action FROM focus_group WHERE group_id=?',(group_id,))
                                    doA = cursor.fetchone()
                                    if doA:
                                        doA = doA[0]
                                        if doA != 'Y':
                                            cursor.execute('UPDATE focus_group SET double_action = "Y" WHERE group_id = ?', (group_id,))  
                                            conn.commit()
                                            # conn.close()
                                            send_reply(reply_token,'กด Y อีกครั้งเพือยืนยันการคิดเงิน')
                                            continue 
                                        else:
                                            cursor.execute('UPDATE focus_group SET double_action = "N" WHERE group_id = ?', (group_id,))  
                                            conn.commit()
                                ...
                                notify_message = f"\nสรุปกำไร\nคู่ที่{round_value}:{int(profit[0])} บาท"
                        ...
                        # elif '!return' == msg or msg.lower() == 'r':
                        #     with sqlite3.connect(db_name) as conn:
                        #         cursor.execute('SELECT game_status FROM focus_group WHERE group_id =?',(group_id,))
                        ...
                        #     continue
                            ...
                            with sqlite3.connect(db_name) as conn:
                                ...
                                if doA:
                                    doA = doA[0]
                                    if doA != 'R':
                                        cursor.execute('UPDATE focus_group SET double_action = "R" WHERE group_id = ?',(group_id,))  
                                        conn.commit()
                                        # conn.close()
                                        send_reply(reply_token,'กด R อีกครั้งเพือย้อนผล')
                                        continue 
                                    else:
                                        cursor.execute('UPDATE focus_group SET double_action = "N" WHERE group_id = ?',(group_id,))  
                                        conn.commit()
                    ...
                    if role and role[0] == 'admin':
                        ...
                        #     send_reply(reply_token,reply_message)
                        #     continue
                        elif 'ปด' == msg or 'p'==msg.lower():
                            
                            with sqlite3.connect(db_name) as conn:
                                cursor = conn.cursor()
                                cursor.execute('SELECT status,game_status FROM focus_group WHERE group_id =?',(main_room,))
                                status_group = cursor.fetchone()
                                if status_group:
                                    if status_group[1]!= 'P':
                                        reply_message = 'ยังไม่ได้ออกราคา กรุณาออกราคาก่อน '
                                ...
                                cursor.execute('UPDATE focus_group SET game_status = "X" WHERE group_id = ?', (main_room,))
                                conn.commit()
                            reply_message = 'เปลี่ยนสถานะห้องเล่นเป็นปิดราคาแล้ว'
                            send_reply(reply_token,reply_message)
                            continue
                        elif 'x'== msg.lower():
                            with sqlite3.connect(db_name) as conn:
                                ...
                                cursor.execute('UPDATE focus_group SET game_status = "XX" WHERE group_id = ?', (main_room,))
                                conn.commit()
                            reply_message = 'เปลี่ยนสถานะห้องเล่นเป็นปิดหมดแล้ว'
                            send_reply(reply_token,reply_message)
                            continue
                        elif 'o' == msg.lower():
                            with sqlite3.connect(db_name) as conn:
                                cursor = conn.cursor()
                                cursor.execute('UPDATE focus_group SET game_status = "O" WHERE group_id = ?', (main_room,))
                                conn.commit()
                            reply_message = 'เปลี่ยนสถานะห้องเล่นเป็นเปิดคู่แล้ว'
                            send_reply(reply_token,reply_message)
                            continue
                    elif status and role[0]== 'user':
                        with sqlite3.connect(db_name) as conn:
                    ...
                    if role and role[0] == 'admin':
                        if role and role[0] == 'admin':
                            ...
                            elif 'ใหม่'==msg or 'เริ่มใหม่'==msg or 'reset'==msg.lower():
        ...
        msg = globals().get("msg", "N/A")
        user_id = globals().get("user_id", "N/A")
        ...
        with open(filename, mode='a', newline='', encoding='utf-8') as file:
            ...
            if not file_exists:
                writer.writerow(['datetime', 'error', 'msg', 'user_id'])

            writer.writerow([error_time, str(e), msg, user_id])
        return 'ok',200
...
if __name__ == '__main__':
    ...
    socketio.run(app,host ='0.0.0.0', port=7030)
